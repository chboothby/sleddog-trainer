...
describe('/api', async function () {
  ...
  describe('/api/kennels/:kennel_id/runs || /api/runs/:run_id', async () => {
    ...
    describe('GET kennels runs', () => {
      ...
      it('should return all runs with total kennel distance key', async () => {
        const {
          body: { runs },
        } = await request(app).get('/api/kennels/1/runs');

        expect(runs).haveOwnProperty('totalDistance');
        expect(runs.totalDistance).to.equal('18');
      });
      it('accepts a filter query for runs from specified date', async () => {
        const {
          body: { runs },
        } = await request(app).get('/api/kennels/1/runs?dateFrom=01012021');

        expect(runs.runs.length).to.eql(0);
      });
      it('accepts a filter query for runs from specified date', async () => {
        const {
          body: { runs },
        } = await request(app).get('/api/kennels/1/runs?dateTo=01012021');
    ...
    describe.only('GET dogs runs', () => {
      ...
      it('accepts a filter query for runs from specified date', async () => {
        const {
          body: { runs },
        } = await request(app).get('/api/dogs/1/runs?dateFrom=01012021');
      ...
      it('accepts a filter query for runs from specified date', async () => {
        const {
          body: { runs },
        } = await request(app).get('/api/dogs/1/runs?dateTo=01012021');
